// export const BasePbrShader = `
//     //
//     struct Uniforms {
//         modelViewMatrix : mat4x4<f32>,
//         projectionMatrix : mat4x4<f32>,
//         // normalMatrix : mat3x3<f32>,  // 改用mat4x4,以满足std140对齐要求,此外关于std430对齐要求,可以查看文档
...
//         // 环境遮蔽
//         aoStrength : f32,
//         padding0 : f32, // 填充至 16 字节（共 4 * f32）

//         // 自发光
//         emissiveColor : vec3<f32>,
//         emissiveIntensity : f32,
//         padding1 : vec3<f32>, // 或 vec3<f32>，但应 padding 成 vec4
...
//         lightColor : vec3<f32>,
//         lightIntensity : f32,
//         padding2 : vec3<f32>, // 或 vec3<f32>，但应 padding 成 vec4
...
//         return pow(c, vec3<f32>(2.4)); // 或 2.4，根据实际情况
//     }
...
//         // output.vNormal = uniforms.normalMatrix * input.aNormal;
...
//         let V = normalize(uniforms.cameraPos - input.vWorldPos);
//         let L = normalize(-uniforms.lightDir);
//         let H = normalize(V + L);

//         // baseColorMap
//         var baseColor = uniforms.baseColor;
//         // baseColor = baseColor * textureSample(uBaseColorMap, uBaseColorSampler, input.vTexCoord).rgb;
//         let sampledColor = textureSample(uBaseColorMap, uBaseColorSampler, input.vTexCoord).rgb;
...
//         // emissive = textureSample(uEmissiveMap, uEmissiveSampler, input.vTexCoord).rgb;
//         var emissive = uniforms.emissiveColor * uniforms.emissiveIntensity;
//         emissive = emissive * textureSample(uEmissiveMap, uEmissiveSampler, input.vTexCoord).rgb;
...
//         let ambient = uniforms.ambientColor * baseColor * ao;

//         let color = ambient + (diffuse + specular) * uniforms.lightColor * uniforms.lightIntensity * NdotL + emissive;

//         return vec4<f32>(color, 1.0);
//         // return textureSample(uBaseColorMap, uBaseColorSampler, input.vTexCoord); // 临时调试,只测试baseColorMap是否正常渲染
//         // return vec4<f32>(mrSample.rgb, 1.0);    // 测试 metallicRoughness 贴图的 G 通道是否影响渲染为绿色
//         // return vec4<f32>(ambient, 1.0); // 测试环境光颜色是否正常
//         // return vec4<f32>(lightColor, 1.0); // 测试平行光颜色是否正常
//         // return vec4<f32>(input.vTexCoord, 0.0, 1.0); // 用颜色查看纹理坐标分布
//     }

//     // PBR辅助函数实现（略，可参考GLSL原版，直接翻译为WGSL语法）
// `;
